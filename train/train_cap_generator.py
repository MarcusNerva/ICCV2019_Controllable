        ...
        for i, (data, caps, caps_mask, cap_classes, class_masks, feats0, feats1, feat_mask, pos_feat, lens, gts, video_id) in enumerate(train_loader):
            ...
            if (i + 1) % opt.save_checkpoint_every == 0:
                if not opt.eval_semantics:
                    current_loss, current_language_state = eval(model, crit, classify_crit, valid_dataset, is_valid=True, eval_kwargs={}.update(vars(opt)))
                else:
                    current_textual_score, current_language_state = eval(model, crit, classify_crit, valid_dataset, is_valid=True, eval_kwargs={}.update(vars(opt)))
